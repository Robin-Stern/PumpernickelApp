...
Output: 9 Kotlin files in `shared/src/commonMain/kotlin/com/pumpernickel/domain/gamification/`. Addresses GAME-01 domain foundation.
</objective>
...
<task type="auto" tdd="true">
  <name>Task 2: Create XpFormula, GamificationEvent, UnlockEvent, EventKeys</name>
  <files>
    shared/src/commonMain/kotlin/com/pumpernickel/domain/gamification/XpFormula.kt,
    shared/src/commonMain/kotlin/com/pumpernickel/domain/gamification/GamificationEvent.kt,
    shared/src/commonMain/kotlin/com/pumpernickel/domain/gamification/UnlockEvent.kt,
  ...
  <action>
    ...
    **File 5: `XpFormulaTest.kt`** under `shared/src/commonTest/kotlin/com/pumpernickel/domain/gamification/XpFormulaTest.kt`
    ```kotlin
    package com.pumpernickel.domain.gamification
...
<task type="auto" tdd="true">
  ...
  <done>
    `AchievementCatalog.kt`, `AchievementProgress.kt`, `AchievementCatalogTest.kt` exist. `Category` has 4 values, `Tier` has 3 values. All 7 tests in `AchievementCatalogTest` pass. `./gradlew :shared:allTests` runs all commonTest tests (RankLadder, XpFormula, AchievementCatalog) without failures.
  </done>
</task>
...
<verification>
- All 9 domain files compile under `./gradlew :shared:compileKotlinMetadata`.
- All three test classes (`RankLadderTest`, `XpFormulaTest`, `AchievementCatalogTest`) pass.
- No Room/Koin/Android/iOS imports in any file under `shared/src/commonMain/kotlin/com/pumpernickel/domain/gamification/` — verify with `grep -rE "import (androidx\\.room|org\\.koin|android\\.|platform\\.)" shared/src/commonMain/kotlin/com/pumpernickel/domain/gamification/` returning zero matches.
...
<success_criteria>
- Pure domain layer with no external dependencies establishes the testable vocabulary for plans 03–09.
...
- `EventKeys` format registry is typed, matching PATTERNS.md dedupe-key table exactly.
</success_criteria>
...
- Confirmation `./gradlew :shared:allTests` passed all gamification tests.
